    ...
    const fetchUserBookings = async () => {
      try {
        ...
        if (bookings) {
          ...
          // Fetch Access-Granted and Door data for each booking
          userBookingsArray.forEach((booking) => {
            // Fetch Access-Granted data
            ...
            // Fetch Door data
            const doorRef = firebase
              .database()
              .ref(`bookings/${booking.id}/Door`);
            doorRef.on("value", (snapshot) => {
              const doorData = snapshot.val();
              // Update the booking with Door data
              setUserBookings((prevUserBookings) =>
                prevUserBookings.map((prevBooking) =>
                  prevBooking.id === booking.id
                    ? { ...prevBooking, doorData }
                    : prevBooking
                )
              );
            });
          ...
          <tbody>
            {userBookings.map((booking) => (
              ...
              >
                ...
                <td className="px-6 py-4">{booking.doorData}</td>
